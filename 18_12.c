...
Student *search(Student **hashTable, int size, int id)
{
    for (int i = 0; i < size; i++)
    {
...
int main()
{
    ...
    getchar(); // Clear the newline character after the last scanf
    ...
    while (fgets(line, sizeof(line), stdin))
    {
        ...
        {
            Student *foundStudent = search(hashTable, m, id);
            if (foundStudent != NULL)
            {
                printf("Found student name %s id %d.\n", foundStudent->name, id);
            }
            else
            {
